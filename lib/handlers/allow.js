...
var oidcOrcid = require("../api/authn/oidc-orcid");
...
function allow (mode) {
  return function allowHandler (req, res, next) {
    ...
    ldp.exists(req.hostname, reqPath, (err, ret) => {
      ...
      if (!userId 
        && oidcOrcid.existsOidcTokenHeaders(req)) {
        debug("Found OIDC X Headers")
        var idToken = req.get("X-OIDC-Token-ID");
        var tokenIssuer = req.get("X-OIDC-Token-Issuer");
        debug("ID Token: " + idToken);
        debug("token issuer: " + tokenIssuer);

        if (oidcOrcid.verifySignature(idToken)) {
          debug("Signature Verified.")
          var contents = oidcOrcid.extractPayloadAsString(idToken);
          userId = contents["orcid"];
        }
      }
      debug("UserId: " + userId);
